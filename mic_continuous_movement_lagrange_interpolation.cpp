...
int main(){
    ...
    vector<float> amplitude_left_storage;
            ...
            for (int i=0; i<total_samples; i++){
                ...
                float y_lagrange = 0.0; // This is our summation result y(x)

                //summation loop for Lagrange interpolation
                for (int k=0; k<=N; k++){
                    int current_k_sample = start_index + k; //this is the sample index for the current k in the summation loop, which corresponds to x_k in the Lagrange interpolation formula

                    float l_k = 1.0; // This is the product term l_k(x) 

                    //product loop for Lagrange interpolation
                    for (int j=0; j<=N; j++){
                        if (j != k) { //we skip the case where j=k in the product term
                            int current_j_sample = start_index + j; //this is the sample index for the current j in the product loop, which corresponds to x_j in the Lagrange interpolation formula
                    ...
                    y_n_left = y_lagrange; //after the summation loop is done, we assign the final result of the Lagrange interpolation to y_n_left, which is the interpolated amplitude value for the left microphone at this global_time.
    ...
    vector<float> amplitude_right_storage;
            ...
            for (int i=0; i<total_samples; i++){
                float global_time = static_cast<float>(i)/static_cast<float>(src.getSampleRate());
                //float x_mic_right = env1.get_x_right() + velocity_right*global_time; //uniform velocity
                ...
                //lagrange interpolation for right mic
                float n_float_right = time_right * src.getSampleRate();
                int M_right = static_cast<int>(floor(n_float_right));
                int N = 3; //cubic interpolation with 4 points
                int start_index = M_right - (N / 2); //center the interpolation around M_right
                ...
                for (int k=0; k<=N; k++){
                    ...
                    y_n_right = y_lagrange; //after the summation loop is done, we assign the final result of the Lagrange interpolation to y_n_left, which is the interpolated amplitude value for the left microphone at this global_time.
