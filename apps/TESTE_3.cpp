//THIS CODE IMPLEMENTATION SIMULATES BOTH THE DELAY PROPAGATION AND THE CONSTRUCTIVE AND DESTRUCTIVE INTERFERENCE OBTAINED IN A STATIONARY MICROPHONE FROM 2 STATIONARY SOURCES, BOTH WITH A CERTAIN DISTANCE FROM THE MICROPHONE.
// TEST 3 IS THE SAME CODE IMPLEMENTATION AS TEST 2,THE ONLY CHANGE IS THE X COORDINATE FOR THE SOURCES
...
int main(){
    ...
    //start with 2 sources
        sources_vec.push_back(Source(1.0, 440.0, 44100, bufferSize, -wavelength_distance, 0, 0));
        sources_vec.push_back(Source(-0.5, 440.0, 44100, bufferSize, 4*wavelength_distance, 0, 0));
