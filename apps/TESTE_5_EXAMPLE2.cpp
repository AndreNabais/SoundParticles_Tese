//THIS CODE IMPLEMENTATION SIMULATES THE DOPPLER EFFECT WHEN A SOURCE APPROACHES A STATIONARY MICROPHONE WITH A CONSTANT ACCELERATION (VELOCITY INCREASES)
...
int main(){
    ...
    int bufferSize = 2048; 
    float total_time = 1.0f; // Total time for the simulation in seconds
    ...
    for (size_t i = 0; i < sources_vec.size(); ++i) {
        ...
        sources_vec[i].setAcceleration(Acceleration(200.0f, 0.0f, 0.0f)); // Set a constant acceleration of 100 m/s² for each source
    }
