//THIS CODE IMPLEMENTATION SIMULATES THE SONIC BOOM PHENOMENON THAT HAPPENS WHEN A SUPERSONIC SOURCE PASSES BY A STATIONARY MICROPHONE AT 1D (ONLY THE X COORDINATE WILL CHANGE)
//FOR THIS TEST, THERE WILL ONLY BE CHANGES FOR THE X COORDINATE (SIMULATING A SUPERSONIC TRAIN), SO THE SONIC BOOM HAPPENS WITH ANY VELOCITY GREATER OR EQUAL THAN THE SPEED OF SOUND
...
int main(){
    ...
    //start with 1 source
    sources_vec.push_back(Source(0.5, source_frequency, 44100, bufferSize, -5.0, 0, 0));
    ...
    for (size_t j = 0; j < sources_vec.size(); ++j) {
        sources_vec[j].setVelocity(Velocity(2.0 * velocity_sound, 0.0f, 0.0f)); // Set a constant velocity of 50 m/s for each source
        ...
        Mach_angle[i][j] = asin(1.0f / Mach_number_x[i][j]) * 180.0f / M_PI; // Convert to degrees

        if (i % 500 == 0) {
            cout << "Mach number for source " << j+1 << " (Sample " << i << "): " << Mach_number_x[i][j] << endl;
            cout << "Mach angle for source " << j+1 << " (Sample " << i << "): " << Mach_angle[i][j] << " degrees" << endl;
            }
        }
    }
